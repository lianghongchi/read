...
$code->warping = 0;//随机扭曲
$code->fontpath = ROOT_PATH . 'core/seccode/font/';//字体包路径
$code->datapath = ROOT_PATH . 'core/seccode/';//背景图片、字体、声音等文件路径
$code->includepath = ROOT_PATH . 'core/';
$code->display();
*/
class Seccode
{
	...
	function image() {
		...
		if($this->animator == 1 && function_exists('imagegif')) {
			include_once $this->includepath.'ImageGifMerge.class.php';
			...
			$anim = new ImageGifMerge($frame, 255, 255, 255, 0, $d, $x, $y, 'C_MEMORY');
	...
	function ttffont() {
		...
		$seccodettf = array();
        function AnofileExt($filename){
            return addslashes(trim(substr(strrchr($filename, '.'), 1, 10)));
        }
		while($entry = readdir($dirs)) {
			if($entry != '.' && $entry != '..' && in_array(strtolower(AnofileExt($entry)), array('ttf', 'ttc'))) {
				$seccodettf[] = $entry;
			}
		}
		if(empty($seccodettf)) {
		...
		if($this->type && !empty($seccodettf)) {
			if(strtoupper(YF_CHARSET) != 'UTF-8') {
				import('@.ORG.Chinese');
				$cvt = new Chinese();
				$seccode = $cvt->Convert(YF_CHARSET, 'UTF-8', $seccode);
			}
        ...
        $mtRandMin = $font[0]['angle'] > 0 ? cos(deg2rad(90 - $font[0]['angle'])) * $font[0]['zheight'] : 1;
        $mtRandMax = $this->width - $widthtotal;
        
        if ($mtRandMin <= $mtRandMax) {
            $x = mt_rand($mtRandMin, $mtRandMax);    
        } else {
            $x = 16;
        }
